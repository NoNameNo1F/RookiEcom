...
namespace RookiEcom.IdentityServer.Migrations.IdentityServer.Configuration
{
    /// <inheritdoc />
    public partial class InitializeConfigurationIdP : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            // Seeding Data
            var clients = OAuthConfig.Clients;
            int clientId = 1;
            foreach (var client in clients)
            {
                ...
                migrationBuilder.InsertData(
                    ...
                    columns: new[]
                    {
                        ...
                        "RequireClientSecret", "RequirePkce", "AllowOfflineAccess",
                        "AccessTokenLifetime", "IdentityTokenLifetime", "Created", "Updated",
                        ...
                        "IncludeJwtId", "AlwaysSendClientClaims", "DeviceCodeLifetime",
                        "NonEditable", "RequirePushedAuthorization"
                    },
                    values: new object[]
                    {
                        clientId, // Id will be auto-generated by the database
                        clientEntity.ClientId,
                        clientEntity.ClientName,
                        clientEntity.Enabled,
                        clientEntity.ProtocolType ?? "oidc",
                        clientEntity.RequireClientSecret,
                        clientEntity.RequirePkce,
                        clientEntity.AllowOfflineAccess,
                        clientEntity.AccessTokenLifetime,
                        clientEntity.IdentityTokenLifetime,
                        clientEntity.Created,
                        clientEntity.Updated,
                        false, // RequireConsent
                        true, // AllowRememberConsent
                        false, // AlwaysIncludeUserClaimsInIdToken
                        false, // AllowPlainTextPkce
                        false, // RequireRequestObject
                        false, // AllowAccessTokensViaBrowser
                        false, // RequireDPoP
                        0, // DPoPValidationMode
                        TimeSpan.Zero, // DPoPClockSkew
                        ...
                        1, // RefreshTokenUsage
                        false, // UpdateAccessTokenClaimsOnRefresh
                        1, // RefreshTokenExpiration
                        0, // AccessTokenType
                        true, // EnableLocalLogin
                        false, // IncludeJwtId
                        false, // AlwaysSendClientClaims
                        300, // DeviceCodeLifetime
                        false, // NonEditable
                        false // RequirePushedAuthorization
                    });

                // Seed ClientRedirectUris
                foreach (var redirectUri in clientEntity.RedirectUris)
                {
                    migrationBuilder.InsertData(
                        table: "ClientRedirectUris",
                        columns: new[] { "ClientId", "RedirectUri" },
                        values: new object[] { clientId, redirectUri.RedirectUri });
                }

                // Seed ClientPostLogoutRedirectUris
                foreach (var postLogoutUri in clientEntity.PostLogoutRedirectUris)
                {
                    migrationBuilder.InsertData(
                        table: "ClientPostLogoutRedirectUris",
                        columns: new[] { "ClientId", "PostLogoutRedirectUri" },
                        values: new object[] { clientId, postLogoutUri.PostLogoutRedirectUri });
                }

                // Seed ClientScopes
                foreach (var scope in clientEntity.AllowedScopes)
                {
                    migrationBuilder.InsertData(
                        table: "ClientScopes",
                        columns: new[] { "ClientId", "Scope" },
                        values: new object[] { clientId, scope.Scope });
                }

                // Seed ClientGrantTypes
                foreach (var grantType in clientEntity.AllowedGrantTypes)
                {
                    migrationBuilder.InsertData(
                        table: "ClientGrantTypes",
                        columns: new[] { "ClientId", "GrantType" },
                        values: new object[] { clientId, grantType.GrantType });
                }

                // Seed ClientSecrets (if any)
                foreach (var secret in clientEntity.ClientSecrets)
                {
                    migrationBuilder.InsertData(
                        table: "ClientSecrets",
                        columns: new[]
            ...
            int scopeId = 1;
            // Seed ApiScopes
            var apiScopes = OAuthConfig.ApiScopes;
            foreach (var scope in apiScopes)
            {
                ...
                migrationBuilder.InsertData(
                    ...
                    columns: new[]
                    {
                        ...
                        "ShowInDiscoveryDocument", "Required", "Emphasize", "Created", "Updated", "NonEditable"
                    },
                    values: new object[]
                    {
                        scopeId,
                        scopeEntity.Name,
                        scopeEntity.DisplayName,
                        scopeEntity.Description,
                        scopeEntity.Enabled,
                        scopeEntity.ShowInDiscoveryDocument,
                        scopeEntity.Required,
                        scopeEntity.Emphasize,
                        scopeEntity.Created,
                        scopeEntity.Updated,
                        false
                    });
                ...
                // Seed ApiScopeClaims
                foreach (var claim in scopeEntity.UserClaims)
                {
                    migrationBuilder.InsertData(
                        table: "ApiScopeClaims",
                        columns: new[] { "ScopeId", "Type" },
                        values: new object[] { scopeId, claim.Type });
                }
            ...
            var identityResources = OAuthConfig.IdentityResources;
            int resourceId = 1;
            foreach (var resource in identityResources)
            {
                ...
                migrationBuilder.InsertData(
                    ...
                    {
                        "Id", "Name", "DisplayName", "Description", "Enabled",
                        "Required", "Emphasize", "ShowInDiscoveryDocument", "Created", "Updated", "NonEditable"
                    },
                    values: new object[]
                    {
                        resourceId, // Id will be auto-generated
                        resourceEntity.Name,
                        resourceEntity.DisplayName,
                        resourceEntity.Description,
                        resourceEntity.Enabled,
                        resourceEntity.Required,
                        resourceEntity.Emphasize,
                        resourceEntity.ShowInDiscoveryDocument,
                        resourceEntity.Created,
                        resourceEntity.Updated,
                        false
                    });

                // Seed IdentityResourceClaims
                foreach (var claim in resourceEntity.UserClaims)
                {
                    migrationBuilder.InsertData(
                        table: "IdentityResourceClaims",
                        columns: new[] { "IdentityResourceId", "Type" },
                        values: new object[] { resourceId, claim.Type });
                }
            ...
            // Seed ApiResources
            var apiResources = OAuthConfig.ApiResources;
            int apiResourceId = 1;
            foreach (var resource in apiResources)
            {
                ...
                migrationBuilder.InsertData(
                    ...
                    {
                        "Id", "Name", "DisplayName", "Description", "Enabled",
                        "ShowInDiscoveryDocument", "RequireResourceIndicator", "Created", "Updated", "NonEditable"
                    },
                    values: new object[]
                    {
                        apiResourceId, // Id will be auto-generated
                        resourceEntity.Name,
                        resourceEntity.DisplayName,
                        resourceEntity.Description,
                        resourceEntity.Enabled,
                ...
                // Seed ApiResourceScopes
                foreach (var scope in resourceEntity.Scopes)
                {
                    migrationBuilder.InsertData(
                        table: "ApiResourceScopes",
                        columns: new[] { "ApiResourceId", "Scope" },
                        values: new object[] { apiResourceId, scope.Scope });
                }
            
                // Seed ApiResourceClaims
                foreach (var claim in resourceEntity.UserClaims)
                {
                    migrationBuilder.InsertData(
                        table: "ApiResourceClaims",
                        columns: new[] { "ApiResourceId", "Type" },
                        values: new object[] { apiResourceId, claim.Type });
                }
